...
class QuizCreate:
    ...
    def run(self):
        ...
        for n, question in enumerate(self.config["questions"]):
            ...
            for outcome, score in self.config["questions"][n]["choices"][answer][
                "impact"
            ].items():
    ...
    def evaluate_outcomes(self) -> str:
        ...
        winning_outcomes = [
            outcome for outcome, score in self.scores.items() if score == max_score
        ]
        ...
        if len(winning_outcomes) > 1:
            match self.config["tiebreaker"]["tiebreaker_method"]:
                case "tiebreaker":
                    ...
                    necessary_choices = [
                        x
                        for x in self.config["tiebreaker"]["choices"]
                        if list(x["impact"])[0] in winning_outcomes
                    ]
                    ...
                    winning_outcomes = [
                        outcome
                        for outcome, score in self.scores.items()
                        if score == max_score
                    ]
...
def main():
    configpath = path.join(
        getcwd(), input(f"Enter config file (relative to {getcwd()}): ")
    )
    quizcreate = QuizCreate(configpath)
    quizcreate.run()
